...
onUnmounted(() => {
  clearInterval(messageStore.timer)
...
function send() {
  ...
  if (msg) {
    messageStore.sendMessage(messageStore.activeChat._id, msg)
  ...
  <div class="mainPage">
    <h1 class="title">
      {{
        messageStore.activeChat
          ? `Chat In ${messageStore.activeChat.group_name}`
          : 'Choose a group to chat with'
      }}
    </h1>
    ...
    <div class="chat-window" ref="chatWindow" v-if="targetFriend">
      <div
        ...
        :class="[
          'message',
          message.sender === messageStore.currentUser.username ? 'right' : 'left',
        ]"
      >
